...
export class LangChainService {
    ...
    async generateResponse(visitorId: string, userMessage: string, history: any[] = []) {
        // Safe history check
...
4. **TONE**:
   - Senior expert, authoritative, extremely helpful, and concise (1-2 sentences).`
        ...
        const chatHistory = safeHistory.map(h =>
            h && h.role?.toLowerCase() === 'user' ? new HumanMessage(h.text || '') : new AIMessage(h.text || '')
        );
    ...
    private async searchProperties(data: any) {
        try {
            return await this.prisma.property.findMany({
                where: {
                    OR: [
                        data.area ? { location: { contains: data.area, mode: 'insensitive' as any } } : {},
    ...
    private extractUserInfoStructured(history: any[], currentMessage: string) {
        ...
        else if (combinedText.includes('buy') || combinedText.includes('sale') || combinedText.includes('purchase')) result.intent = 'SALE';
        ...
        if (sectorsFound) result.area = Array.from(new Set(sectorsFound)).join(', ').toUpperCase();

        if (combinedText.includes('house')) result.propertyType = 'RESIDENTIAL';
        else if (combinedText.includes('apartment') || combinedText.includes('flat')) result.propertyType = 'RESIDENTIAL';
        else if (combinedText.includes('office') || combinedText.includes('shop') || combinedText.includes('commercial')) result.propertyType = 'COMMERCIAL';
